...
class DataPreprocessingReady():
  ...
  def get_history(self, data, uid_set):
    ...
    for uid in tqdm.tqdm(uid_set):
      ...
      all_seq_dict[uid] = pos
      pos = data[(data.uid == uid) & (data.y > 3)].iid.values.tolist()
      pos_seq_dict[uid] = pos
      pos = data[(data.uid == uid) & (data.y < 4)].iid.values.tolist()
      neg_seq_dict[uid] = pos

    return all_seq_dict, pos_seq_dict, neg_seq_dict
  ...
  def split(self, src, tgt):
    ...
    all_seq_dict, pos_seq_dict, neg_seq_dict = self.get_history(src, co_users)
    ...
    train_meta['all_seq'] = train_meta['uid'].map(all_seq_dict)
    test['pos_seq'] = test['uid'].map(pos_seq_dict)
    test['neg_seq'] = test['uid'].map(neg_seq_dict)
    test['all_seq'] = test['uid'].map(all_seq_dict)

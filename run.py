...
class Run():
  def __init__(self, config):
    ...
    self.results = {'tgt_mae': 10, 'tgt_rmse': 10, 'aug_mae': 10, 'aug_rmse': 10, 'emcdr_mae': 10, 'emcdr_rmse': 10, 'dupgtcdr_mae': 10, 'dupgtcdr_rmse': 10}
  ...
  def read_log_data(self, path, batchsize, history=False):
    if not history:
      ...
      data = pd.read_csv(path, header=None, usecols=[0,1,2,3,4])
      ...
      neg_seq = keras.preprocessing.sequence.pad_sequences(data.neg_seq.map(self.seq_extractor), maxlen=20, padding='post')
  ...
  def read_map_data(self):
    ...
    data = pd.read_csv(self.meta_path, header=None, usecols=[0,1,2,3])
    data.columns = cols
  ...
  def read_aug_data(self):
    ...
    x_col = ['uid', 'iid']
    y_col = ['y']
    src = pd.read_csv(self.src_path, header=None, usecols=[0,1,2])
    src.columns = cols_train
    tgt = pd.read_csv(self.tgt_path, header=None, usecols=[0,1,2])
  ...
  def get_model(self):
    model = Model(self.uid_all, self.iid_all, self.num_fields, self.emb_dim)
  ...
  def get_optimizer(self, model):
    ...
    optimizer_meta = torch.optim.SGD(params=model.dupgt.parameters(), lr=self.lr, weight_decay=self.wd, momentum=0.9)
  ...
  def CDR(self, model, data_src, data_map, data_meta, data_test, criterion, optimizer_src, optimizer_map, optimizer_meta):
    ...
    for i in range(self.epoch):
      ...
      print('MAE: {} RMSE: {}'.format(mae, rmse))
    print('==========DUPGT-CDR==========')
    scheduler_DUPGTCDR = torch.optim.lr_scheduler.OneCycleLR(optimizer_meta, max_lr=0.02, epochs=self.epoch, steps_per_epoch=len(data_meta), pct_start=0.2)
    for i in range(self.epoch):
      self.train(data_meta, model, criterion, optimizer_meta, scheduler_DUPGTCDR, i, stage='train_meta')
      mae, rmse = self.eval_mae(model, data_test, stage='test_meta')
      self.update_results(mae, rmse, 'dupgtcdr')
      print('MAE: {} RMSE: {}'.format(mae, rmse))
  ...
  def main(self):
    ...
    print("\nFinal Results:")
    print("TgtOnly: MAE = {:.4f}, RMSE = {:.4f}".format(
        self.results['tgt_mae'], self.results['tgt_rmse']))
    print("DataAug: MAE = {:.4f}, RMSE = {:.4f}".format(
        self.results['aug_mae'], self.results['aug_rmse']))
    print("EMCDR: MAE = {:.4f}, RMSE = {:.4f}".format(
        self.results['emcdr_mae'], self.results['emcdr_rmse']))
    print("DUPGT-CDR: MAE = {:.4f}, RMSE = {:.4f}".format(
        self.results['dupgtcdr_mae'], self.results['dupgtcdr_rmse']))
